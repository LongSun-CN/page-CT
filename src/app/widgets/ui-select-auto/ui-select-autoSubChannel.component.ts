...
export class UiSelectAutoSubChannelComponent implements OnInit,OnDestroy,OnChanges {
  @Input()
  ...
  ngOnInit() {
    this.$subscription = this.$store.select('subChannel').map((state:serviceState[])=>{
